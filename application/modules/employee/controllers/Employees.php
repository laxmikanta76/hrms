...
//print_r($postData);exit();
	...
	public function update_employee_form($id = null)
{
    $this->permission->method('employee', 'update')->redirect();
    ...
    if ($this->form_validation->run() === TRUE) {
        ...
        $this->Employees_model->update_employee($postData);

        // update custom fields
        $this->db->where('employee_id', $employee_id)->delete('custom_table');
        $custom_fields = $this->input->post('c_f_name');
        $custom_types  = $this->input->post('c_f_type');
        $custom_values = $this->input->post('customvalue');
            ...
            foreach ($custom_fields as $i => $field) {
                if (!empty($field)) {
                    $this->db->insert('custom_table', [
                        'employee_id'      => $employee_id,
                        'custom_field'     => $field,
                        'custom_data_type' => $custom_types[$i],
                        'custom_data'      => $custom_values[$i]
                    ]);
                }
            }
        }
        ...
        // update benefits
        $this->db->where('employee_id', $employee_id)->delete('employee_benifit');
        $ben_codes = $this->input->post('benifit_c_code');

        if (!empty($ben_codes)) {
            foreach ($ben_codes as $i => $code) {
                if (!empty($code)) {
                    $this->db->insert('employee_benifit', [
                        'employee_id'      => $employee_id,
                        'bnf_cl_code'      => $code,
                        'bnf_cl_code_des'  => $this->input->post('benifit_c_code_d')[$i],
                        'bnff_acural_date' => date(
                            'Y-m-d',
                            strtotime($this->input->post('benifit_acc_date')[$i])
                        ),
                        'bnf_status'       => $this->input->post('benifit_sst')[$i]
                    ]);
                }
            }
        }
        ...
        redirect('employee/Employees/cv/' . $employee_id);
    }
