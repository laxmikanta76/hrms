...
public function create_employee_payment($data = array())
	...
	public function salary_paymentinfo($id = null){
			return $this->db->select('count(DISTINCT(pment.emp_sal_pay_id)) as emp_sal_pay_id,pment.*,p.employee_id,p.first_name,p.last_name,p.bank_name,p.bank_account_no,p.pan_number,p.pf_number,p.uan_number,p.joining_date,dept.department_name,desig.position_name,p.rate as basic,p.rate_type as salarytype')   
